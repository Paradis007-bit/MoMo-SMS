...
_Base URL:_ http://localhost:8000  
_Authentication:_ Basic Auth — username: admin, password: password123
...
_Linear search_ loops through every record one by one until it finds a match. If the record is near the end, it takes longer. Time complexity: O(n).

_Dictionary lookup_ builds a hash map from all transaction IDs first, then looks up any record directly. It does not matter how large the dataset is. Time complexity: O(1).
